...
def stage1A_accumulate_AS(
    ...
    if isinstance(ekfac, EKFAC_QK_Head):
        ...
        hooks = setup_qk_hooks(model_ddp.module, layer_idx, cache, verbose=verbose)
    elif isinstance(ekfac, EKFAC_QKVO_Head):
        cache = QKVOActivationCache()
        hooks = setup_qkvo_hooks(model_ddp.module, layer_idx, cache, verbose=verbose)
    else:
    ...
    for batch_idx, (batch_tokens, _) in enumerate(tqdm(dataloader, desc="Accum A/S", disable=not verbose)):
        ...
        cache.clear()
        if device.startswith("cuda") and ((batch_idx + 1) % empty_cache_every == 0):
    ...
    try:
        model_ddp.module.reset_hooks(hooks)
    except Exception:
        try:
            model_ddp.module.remove_all_hook_fns()
        except Exception:
            pass
...
def stage1B_fit_lambda(
    ...
    if isinstance(ekfac, EKFAC_QK_Head):
        cache = QKActivationCache()
        hooks = setup_qk_hooks(model_ddp.module, layer_idx, cache, verbose=False)
        ...
        cache = QKVOActivationCache()
        hooks = setup_qkvo_hooks(model_ddp.module, layer_idx, cache, verbose=False)
        lambda_sum = [
