...
def compute_sample_grads_qkonly(model_ddp, npy_batch, layer_idx, head_idx, seq_length, dtype, device):
    """
...
def compute_sample_grads_qkvo(model_ddp, npy_batch, layer_idx, head_idx, seq_length, dtype, device):
    """
...
def phase2_score_qkonly(
    ...
    for _, (batch_tokens, batch_indices) in enumerate(tqdm(dataloader, desc="Scoring", disable=not verbose)):
        ...
        g0, loss_val = compute_sample_grads_qkonly(
            model_ddp, batch_tokens, layer_idx, head_idx, seq_length, dtype, device
        )
        ...
        if device.startswith("cuda") and (processed_samples % empty_cache_every == 0):
